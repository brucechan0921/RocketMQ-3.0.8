...
public class MapedFile extends ReferenceResource {
    ...
    // 映射的起始偏移量
    /**
     * chen.si：整个抽象队列中，该文件开头所处的offset。 即使之前的文件删除了，offset仍然保留
     */
    ...
    public MapedFile(final String fileName, final int fileSize) throws IOException {
        ...
        try {
        	/**
        	 * chen.si: 其实这里的file channel可以关闭掉了
        	 * 
        	 * 好吧，index存储还需要这个channel，其他存储都不需要，也就文件表的该文件ref+1
        	 */
    ...
    /**
     * chen.si: 再提供一下这种方法吧，别把queue offset和file offset到处泄露了
     * 
     * public long getQueueOffset() {
     * 	return this.fileFromOffset + this.wrotePostion.get();
     * }
     */
